...
This is the app's core differentiator, so it's covered in depth here, with the backend setup broken out further into `10-VPS-WHISPER-SETUP.md` and `11-TRANSLATION-STRATEGY.md`.

## Goal

Let a user generate subtitles, in a language of their choosing, for whatever is currently playing — local file, IPTV channel, or user-added live stream — using a Whisper Large model.
...
Audio obtained:
  ...
  - IPTV/LIVE: rolling buffer (e.g., last 20–30s)
        │
        ▼
Sent to backend /transcribe endpoint (chunked for live sources)
        │
        ▼
Backend: Whisper Large transcribes → (if needed) translates → returns timed segments
        │
        ▼
...
- **Cue conversion**: turning `{start, end, text}` segments from the backend into a renderable subtitle cue list, with interpolation/timing adjustment if there's slight drift between segment timestamps and actual playback position.
...
- Visible loading/progress state while the first segment is being generated — this can take a few seconds depending on backend load, and users need to know it's working, not stuck.
...
- Clear, specific error messaging if the backend is unreachable, times out, or returns an error — distinguishing "can't reach the server" from "server returned an error" helps the developer debug the setup too.
...
- **Rolling buffer size** for live sources — trade-off between subtitle latency (smaller buffer = faster but more requests/backend load) and transcription accuracy (Whisper tends to do better with more context per chunk).
- **Retry/backoff policy** if a backend request fails mid-stream for live subtitles — silently retry the next chunk, or surface an error immediately?
- **Rate limiting** — even for a single-user personal app, it's worth deciding whether the backend should reject overlapping requests (e.g., if the user rapidly toggles subtitles on/off) rather than queuing up redundant work.
