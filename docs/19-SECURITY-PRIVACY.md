...
- **Backend API key** — grants access to the subtitle inference server.
...
## Storing the backend API key

- Store in EncryptedSharedPreferences, not in a plain settings file or, worse, hardcoded in source (hardcoding is especially risky since this key grants access to the server).
- Never log the API key (see `17-ERROR-HANDLING-LOGGING.md`) — redact it even in debug logging.

## Transport security

- All communication with the backend **must** be over HTTPS/TLS — audio content and credentials should never transit in plaintext. This means setting up a real TLS certificate (e.g., via Let's Encrypt) — the traffic still crosses the public internet.
...
- Audio from the user's local files or live streams is sent to the backend for processing — it's worth being transparent about in-app (e.g., a brief note in Settings near the configuration: "Audio is sent to your configured server for subtitle generation") so it's never a surprise to whoever ends up using the app.
...
- [ ] Backend API key encrypted at rest, never logged
- [ ] All backend communication over HTTPS with a valid certificate
