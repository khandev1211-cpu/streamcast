...
| Networking | Retrofit + OkHttp | For VPS API calls, IPTV playlist/EPG fetches; OkHttp interceptors handle auth headers, logging, retries |
...
| Queueing (optional, if load increases) | Simple job queue (e.g., Redis + RQ/Celery) | Keeps concurrent requests from overwhelming a single VPS's inference capacity |
